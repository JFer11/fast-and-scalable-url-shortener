...
class TestUser:
    ...
    async def check_user_token(self, token: str, session: AsyncSession) -> None:
        """
        ...
        user_from_token = await AuthManager().get_user_from_token(token, session)
        assert user == user_from_token
    ...
    async def test_signup(self, client, session) -> None:
        expected_expire = datetime.utcnow() + timedelta(
            minutes=settings.access_token_expire_minutes
        )
        response = await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        ...
        await self.check_user_token(token=token, session=session)

    async def test_signup_wrong_shema(self, client) -> None:
        response = await client.post(self.SIGNUP_URL)
        assert response.status_code == 422
    
    async def test_signup_dup_emails(self, client) -> None:
        await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        response = await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        assert response.status_code == 409
        data = response.json()
        assert data["detail"] == "Email address already in use"
    
    async def test_login(self, client, session) -> None:
        await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        response = await client.post(self.LOGIN_URL, json=self.TEST_PAYLOAD)
        expected_expire = datetime.utcnow() + timedelta(
            minutes=settings.access_token_expire_minutes
        )
        assert response.status_code == 200
    ...
    async def test_login_wrong_shema(self, client) -> None:
        response = await client.post(self.LOGIN_URL)
        assert response.status_code == 422

    async def test_login_fail(self, client) -> None:
        response = await client.post(self.LOGIN_URL, json=self.TEST_PAYLOAD)
        self.check_login_fail(response=response)

    async def test_login_bad_password(self, client) -> None:
        await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        payload = {"email": self.TEST_EMAIL, "password": "oops"}
        response = await client.post(self.LOGIN_URL, json=payload)
        self.check_login_fail(response=response)

    async def test_me_header(self, client) -> None:
        sign_up_resp = await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        data = sign_up_resp.json()
        token = f"Bearer {data['access_token']}"
        client.cookies.clear()
        response = await client.get(self.ME_URL, headers={AuthManager.header_name: token})
        self.check_me_response(response=response)

    async def test_me_cookie(self, client) -> None:
        await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
    ...
    async def test_me_bad_access_token(self, client) -> None:
        await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        client.cookies.clear()
    ...
    async def test_me_bad_cookie(self, client) -> None:
        await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        client.cookies.clear()
    ...
    async def test_expired_token(self, client) -> None:
        await client.post(self.SIGNUP_URL, json=self.TEST_PAYLOAD)
        time.sleep(3)
        response = await client.get(self.ME_URL)
        assert response.status_code == 401
